...
class A {
	...
	void execute() {
		...
		// 인터페이스
		// 1) 기능의 틀을 정의하고
		// 2) 이를 틀을 사용할 수 있는 여러개의 클래스를 만든뒤
		...
		// default 메소드 : 객체지향적이지 않음(데이터가 없고 기능만 있음)
		// 클래스 분리 : 교체가 필요한 기능만 인터페이스로 이동, 코드가 복잡해짐
		// 유틸 클래스 : 응집도 관점에서 효과적이지 못함, 코드가 복잡해짐

		// 파급효과를 줄이기 위해 인터페이스를 사용했지만
		// 오히려 개발 생산성이 낮아지는 문제가 발생

		// 1) 파급효과와 이를 해결하기 위한 비용은 그대로 남아 있음
		// 2) 요구사항에 따라 인터페이스는 계속 변경, 관련된 구현 코드도 계속 수정
		// 3) 인터페이스와 구현클래스가 1:N 인 요구사항은 없음

		// 인터페이스와 구현클래스가 1:1 관계일땐 인터페이스가 필요 없음
